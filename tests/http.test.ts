...
import { KengineSDK, betterHttpInstrumentation } from "../src/index";
...
describe("Test KengineSDK for opentelemetry", async () => {
    ...
    it.skip("HTTP Vercel HEADERS!", async () => {
        ...
        expect(traceRequest.headers["x-api-key"]).toBe(kengineKey);
        

        const { resourceSpans: [serialisedSpan]} = getSpans(traceRequest)

        console.log(JSON.stringify(serialisedSpan, null, 2))
        expect(serialisedSpan.resource.attributes.find((attr) => attr.key === "$kengine.service")?.value.stringValue).toBe("my_service");
        expect(serialisedSpan.resource.attributes.find((attr) => attr.key === "$kengine.namespace")?.value.stringValue).toBe("my_namespace")
    })
});
